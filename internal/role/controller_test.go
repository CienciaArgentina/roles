...
func (s *ServiceMock) GetAssignedRole(id int64) (*AssignedRole, error) {
	if s.FailAll {
		return nil, errors.New("Error")
	}
...
// AssignRole MOCK
func (s *ServiceMock) AssignRole(authID int64, roleID int) error {
	if s.FailAll {
...
func (s *ServiceMock) DeleteAssignedRole(authID int64) error {
	if s.FailAll {
		return errors.New("Error")
	}
	return nil
...
func TestControllerImpl_AssignRole(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			requestBody: `{"auth_id": 1, "role_id": 1}`,
